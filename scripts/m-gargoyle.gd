...
#SIGNAL-gargoyle-1 Define the signal with parameters able to destroy a block when hit
...
#	detect_range = 80.0     # how far ahead the gargoyle can "see"
	detect_range = GameConfig.monsterdata[family].detect_range

	hitbox = get_node_or_null("HitBox")
	_setup_hitbox()
#	print("gargoyle hitbox found: ", hitbox)
###DEBUG Hitbox
#	print("gargoyle HitBox monitorable=", hitbox.monitorable, " monitoring=", hitbox.monitoring,
...
func _process_patrol(_delta):
	...
	if is_on_wall():
		#SIGNAL-gargoyle-2 emits defined signal when it hits the wall
		# Pass the gargoyle's own position (like the player's cast does).
...
func _process_breathing(_delta):
	...
	_apply_gravity(_delta)
	move_and_slide()
	# Returns to PATROL automatically once the "gargoyle" animation
...
func _start_charge():
	...
	change_state("gargoyle_charge")  # add this state to monster.cfg (see notes)


 
func _breathe_fire():
	breath_state = BreathState.BREATHING
	change_state("gargoyle_breath")  # add this state to monster.cfg, loop = false
 
	if not fireball_scene:
		push_warning("Gargoyle '%s' has no fireball_scene assigned - can't breathe fire!" % name)
		return
	...
	var spawn_distance = 40.0  # tweak to match the gargoyle's mouth/sprite pivot

 
	# Spawn offset uses -direction (not +direction). The math for "front"
	# was inverted relative to the gargoyle's sprite/pivot, which is why the
